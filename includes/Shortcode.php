...
class Shortcode
{
    ...
    {
        $this->theme_css = $this->get_theme_handler($this->settings['highlight-code']['theme']['values'], $theme);

        wp_enqueue_script('rrze-typesettings');
        wp_enqueue_style('rrze-typesettings');

        wp_enqueue_style($this->theme_css);
    ...
    private function get_theme_handler($theme_options, $theme = '')
    {
        if (is_null($theme)) {
            return 'prismjs-default';
        }
